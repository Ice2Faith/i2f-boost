package i2f.extension.spring.springboot.config.asyn;
 ...
 * @desc
 */
@ConditionalOnExpression("${i2f.springboot.config.async.enable:true}")
@Slf4j
@Data
@NoArgsConstructor
@EnableAsync
@Configuration
@ConfigurationProperties(prefix = "i2f.springboot.config.async")
public class AsyncConfig implements AsyncConfigurer {
    ...
    public ThreadPoolTaskExecutor executorPool() {
        ...
        //线程初始化
        executor.initialize();
        log.info("AsyncConfig ThreadPoolTaskExecutor config done");
        return executor;
    }
