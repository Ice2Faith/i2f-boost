...
func GetConfigApplication(config *GobootConfig, listener *GobootLifecycleListener) *GobootApplication {
	...
	if len(server.Proxy) > 0 {
		LogInfo("goboot enable %v proxy(s)", len(server.Proxy))
		for _, item := range server.Proxy {
	...
	// 配置自动映射
	if len(server.Mapping) > 0 {
		LogInfo("goboot enbale %v mapping(s)", len(server.Mapping))
		for _, item := range server.Mapping {
...
// 处理代理请求
func ProxyHandler(c *gin.Context, proxy Proxy, proxyPath string) {
	url, err := url.Parse(proxy.Redirect)
	if err != nil {
		panic(err)
	}
