...
public class IfGenerate implements IGenerate {
    ...
    public static boolean getCondResult(Triple<String,String,String> trp,Object item){
        ...
        if("==".equals(ope)){
            ...
            // can to date
            if(ConvertResolver.isValueConvertible(left,Date.class)
            && ConvertResolver.isValueConvertible(right,Date.class)){
                Object lv=ConvertResolver.tryConvertible(left,Date.class);
                Object rv=ConvertResolver.tryConvertible(right,Date.class);
                return lv.equals(rv);
            }
            ...
            if(ConvertResolver.isValueConvertible(left,Date.class)
                    && ConvertResolver.isValueConvertible(right,Date.class)){
                Date lv=(Date)ConvertResolver.tryConvertible(left,Date.class);
                Date rv=(Date)ConvertResolver.tryConvertible(right,Date.class);
                return lv.compareTo(rv)>0;
            }
            // can to string,or other obj
