 ...
 * @desc
 */
public class Object2IntegerConverter implements ITypeConverter {

    @Override
    public boolean support(Object obj, Class<?> tarType) {
        if (!TypeUtil.instanceOfAny(tarType, Integer.class, int.class)) {
            return false;
        }
        if (obj == null) {
            return true;
        }
        boolean ok = TypeUtil.instanceOfAny(obj.getClass(), Integer.class, int.class);
        if (ok) {
            return true;
        }
        try {
