...
public class CmdLineExecutor {
    ...
    public static Process exec(String cmdLine,String[] env,File dir,boolean wait,@CloudBe("-1") long timeout,@Nullable TimeUnit timeUnit){
    ...
    public static Process run(String cmdLine, String[] env, File dir, boolean wait, @CloudBe("-1") long timeout, @Nullable TimeUnit timeUnit) throws IOException, InterruptedException {
        Process process = getRuntime().exec(cmdLine, env, dir);
        if (wait) {
            if(timeout>0){
